...
const TVMoviesScreen: React.FC<TVMoviesScreenProps> = ({ navigation, focusEntryRef }) => {
    ...
    const {
        ...
        hasMore,
        loadMore,
    } = usePagedCatalog<XtreamMovie>({
        categories: moviesCategories,
        fetchPage: (api, page, limit, categoryId) => api.getVodStreamsPage({ page, limit, categoryId }),
        ...
        shuffleItems: (items, categoryId, page) => seededShuffle(
            ...
            `movies:${categoryId}:page:${page}:${dayShuffleSeed}`
        ),
        countNoun: 'titles',
        allCategoryName: 'All Movies',
        resetKey: `${moviesLoaded ? 'ready' : 'pending'}:${maxRating}:${isKidsMode ? 'kids' : 'adult'}`,
    });
        ...
        <View style={styles.container}>
            ...
            <View style={styles.moviesPanel}>
                <View style={styles.gridHeader}>
                    ...
                    <Text style={styles.movieCount}>{countLabel}</Text>
                </View>
