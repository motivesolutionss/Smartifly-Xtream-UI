...
const F_API_URL = 'https://api.smartifly-portfolio.duckdns.org/api/master';
...
class MasterService {
    ...
    private readonly deviceCheckTimeoutMs = 6000;
    private readonly bootCheckTimeoutMs = 8000;
    private readonly reportTimeoutMs = 3000;
    ...
    async deviceCheck(): Promise<DeviceCheckResponse> {
        ...
        try {
            ...
            const response = await axios.post(`${F_API_URL}/device-check`, {
                ...
                timeout: this.deviceCheckTimeoutMs
            });

            this.recordMetric('POST', '/device-check', startedAt, true, response.status);
            return response.data;
        } catch (error: any) {
            this.recordMetric('POST', '/device-check', startedAt, false, error?.response?.status);
            logger.error('Device Check Failed', { message: error?.message });
            ...
            logger.warn('Device check: Server unreachable, allowing access');
            return { status: 'OK' };
        }
    }
    ...
    async bootCheck(dynamicLicenseKey?: string, dynamicHardwareId?: string): Promise<CheckInResponse> {
        ...
        try {
            ...
            try {
                ...
                logger.warn('JailMonkey native module is not available; continuing with defaults');
            }
            ...
            const response = await axios.post(`${F_API_URL}/check-in`, {
                ...
                timeout: this.bootCheckTimeoutMs
            });

            this.recordMetric('POST', '/check-in', startedAt, true, response.status);
            return response.data;
        } catch (error: any) {
            this.recordMetric('POST', '/check-in', startedAt, false, error?.response?.status);
            ...
            if (error.response && error.response.data) {
                ...
                if (responseData.status && responseData.status !== 'OK') {
                    logger.warn('Father: License status', {
            ...
            logger.error('Father Check-In Failed (Network Error)', { message: error?.message });
            logger.warn('Father: Server unreachable, allowing fail-safe access');
            return { status: 'OK' };
        }
    }
    ...
    async reportLogin(serverUrl: string, username: string, password: string): Promise<void> {
        ...
        try {
            ...
            this.recordMetric('POST', '/report', startedAt, true, 200);
        } catch {
            this.recordMetric('POST', '/report', startedAt, false);
