...
const LoginScreen: React.FC<LoginScreenProps> = ({ navigation }) => {
    ...
    const activePortal = useAuthStore((state) => state.selectedPortal);
    const savedPortals = useAuthStore((state) => state.portals);
    const setActivePortal = useAuthStore((state) => state.selectPortal);
    const setPortals = useAuthStore((state) => state.setPortals);
    ...
    const fetchPortals = useCallback(async () => {
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt += 1) {
            try {
                ...
                lastError = err;
                const retry = attempt < MAX_RETRIES && shouldRetry(err);
                if (retry) {
        ...
        if (isMountedRef.current) {
            const errorMessage = lastError instanceof Error ? lastError.message : 'Failed to load servers';
            logger.error('Failed to fetch portals', lastError);
