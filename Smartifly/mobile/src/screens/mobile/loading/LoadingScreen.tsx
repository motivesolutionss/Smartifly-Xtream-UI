...
const LoadingScreen: React.FC<LoadingScreenProps> = ({ navigation }) => {
    ...
    const prefetchAllContent = useContentStore((state) => state.prefetchAllContent);
    ...
    const isRetrying = useContentStore((state) => state.isRetrying);
    const retryCount = useContentStore((state) => state.retryCount);
    const maxRetries = useContentStore((state) => state.maxRetries);
    ...
    const handleRetry = useCallback(async () => {
        ...
        useContentStore.setState({ retryCount: 0 });
        await startPrefetch();
    }, [startPrefetch]);
                            ...
                            <View key={step.number} style={styles.stepWrapper}>
                                <View style={[
                                    ...
                                    {status === 'active' ? (
                                        ...
                                        <Icon name={step.icon} size={15} color={colors.textMuted} />
                                    )}
                                </View>
