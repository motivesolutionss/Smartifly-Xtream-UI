...
const ProfileSwitcherScreen: React.FC<ProfileSwitcherScreenProps> = ({ navigation }) => {
    ...
    const handleSwitchSuccess = useCallback(() => {
        // If ProfileSwitcher was opened from inside the app, return to previous screen.
        if (navigation.canGoBack()) {
            navigation.goBack();
            return;
        }

        // If it is the first route (startup profile selection), jump into app shell.
        navigation.reset({
            index: 0,
    ...
    }, [handleSwitchSuccess, isEditMode, navigation, switchProfile]);
    ...
    }, [handleSwitchSuccess, selectedProfileId, switchProfile]);
