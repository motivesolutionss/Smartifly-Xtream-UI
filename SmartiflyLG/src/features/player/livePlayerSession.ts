...
/** Mirrors the Samsung player: channel switching should be immediate. */
export const LIVE_CHANNEL_MIN_HANDOFF_MS = 0;
export const LIVE_CHANNEL_REVISIT_COOLDOWN_MS = 0;
export const LIVE_SWITCH_FORBIDDEN_RETRY_MS = 0;
...
export function markLiveStreamStopped(streamId: number) {
  // Samsung AVPlay does not treat a previously left channel as temporarily blocked.
  liveStreamLastStopTimes.delete(streamId);
}

export function markLiveStreamForbidden(streamId: number, cooldownMs: number) {
  // Keep the API shape, but do not create a revisit lockout.
  liveStreamForbiddenUntil.delete(streamId);
}

export function getLiveHandoffDelayMs(streamId: number) {
  return LIVE_CHANNEL_MIN_HANDOFF_MS;
}
...
export function clearLiveStreamMemory(streamId: number | null) {
  if (streamId != null) {
    liveStreamLastStopTimes.delete(streamId);
    liveStreamForbiddenUntil.delete(streamId);
  }
