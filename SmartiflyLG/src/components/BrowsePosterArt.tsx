...
export function BrowsePosterArt({
  ...
  // Smart check to format ratings as "Rating: 5" versus static video badges like "HD"
  const isNumericBadge = badge && !isNaN(Number(badge));
  const badgeText = isNumericBadge ? (
    <>
      Rating: <strong style={{ color: '#E50914', fontWeight: 800 }}>{badge}</strong>
    </>
  ) : (
    badge
  );
  ...
  return (
    <div style={mergeStyle(artStyle, { position: 'relative', overflow: 'hidden' })}>
      {/* 1. Instant Fallback Layer */}
      {isLive ? (
        ...
        >
          {isLive ? (
            ...
            >
              ...
              {/* Rating/Quality Badge pinned absolutely to the bottom-left */}
              <span
                style={{
                  position: 'absolute',
                  bottom: '18px',
      ...
      {/* 3. High-Quality Loaded Image Layer */}
      {loadedSrc && (
        <img
          src={loadedSrc}
          alt=""
          style={mergeStyle(imgStyle, {
            position: 'absolute',
            top: 0,
            right: 0,
            bottom: 0,
